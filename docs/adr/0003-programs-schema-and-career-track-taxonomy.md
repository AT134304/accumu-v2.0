...
> **⚠️ 2026-08-09 — taxonomy 값 집합은 `docs/adr/0014-taxonomy-rebuild.md`가 대체한다.**
> `program_category`는 8종 → **4종**(`school`/`contest`/`volunteer`/`career`, 교내·교외 축 폐지),
> `career_track`은 5종 → **7종**(`hum`/`soc`/`biz`/`sci`/`eng`/`med`/`art`, `it`은 `eng`에 흡수).
>
> **이 ADR의 구조적 결정은 전부 그대로 유효하다** — enum 3종으로 간다는 결정 1, 두 컬럼이 한 타입을
> 공유한다는 결정 3(오히려 0014가 그 덕에 "한쪽만 옮기면 타입을 지울 수 없다"를 확인했다),
> 인덱스를 만들지 않는다는 판단, 시드 설계 요구사항이 전부 남는다. 바뀐 것은 **값 목록뿐**이다.

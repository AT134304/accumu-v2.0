...
- **모든 클릭 단계에 하이라이트가 있는 건 아니다.** "QR 확인 버튼을 누르고 입장 QR을 열어보세요"
  (5단계)처럼 두 번의 클릭이 필요한 지시는 마지막(결정적) 클릭만 하이라이트한다 — 중간 버튼(QR 확인)
  까지 하이라이트하면 클릭 시점에 따라 단계가 먼저 넘어가 버리는 문제가 생겨서다. 안내 문구가 그
  중간 클릭을 대신 말해준다.

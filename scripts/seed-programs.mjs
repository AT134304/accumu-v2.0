...
//   - "경기도 공유학교…"/"OO대학교 의학계열…" = 기간제 프로그램 2건(20260810 추가, 17→19건.
//     이후 확정 F 상한이 없어져 이 숫자 자체는 더 이상 의미 있는 제약이 아니다 — 아래 참고).
...
const DEMO_PROGRAMS = [
  ...
  //   그 자리를 채우려고 새 행을 만들지 않았다 — 17건(당시)도 확정 F(16개 이상) 조건을 만족했다.
  ...
  // --- 교내 활동 (school) — 자율활동 ---
  {
    ...
    //   비지 않게 하려고 dayOffset 2 -> 0 으로 옮겼다(행을 추가하지 않는다).
...
function assertSeedInvariants() {
  ...
  // [확정 F 개정 — 2026-08-10, 케빈 요청 "프로그램 총 개수 상한은 정하지 마"]
  //   원래 확정 F는 16~20개 범위였다. 상한을 없앴다 — 시드가 계속 늘어나도(다양한 계열 보강 등)
  //   이 스크립트가 막을 이유가 없다는 판단이다. 하한 16은 유지한다 — 그 아래로는 홈 카드/계열 분포
  //   같은 다른 인수 조건(아래)이 애초에 성립하기 어렵다.
  if (rows.length < 16) {
    problems.push(`프로그램 수가 ${rows.length}건 — 확정 F(최소 16개) 위반`);
  }

...
| `program_reports` | id, program_id, student_id, reason, detail | 학생의 프로그램 신고 (ADR 0025). **서로 다른 학생 3명**이면 서버가 자동으로 게시중단(삭제 아님). 학생 1명당 1건(unique). **사유가 두 종류다(ADR 0026 개정 1)**: 참여자 전용은 `mismatch`·`unpunctual` **2종뿐**이고(`status in (entered, completed)` 인 학생만), 나머지는 전부 공개다. 가르는 기준은 **"참여 인증(QR)이 실제로 남을 수 있는 일인가"** — `not_real`(안 열림)은 갔더라도 QR 을 찍을 수 없어서 참여자 전용으로 두면 아무도 못 쓰는 죽은 사유가 된다(실제로 그랬다). >>> 새 사유를 추가할 때 먼저 물을 것: "이 일이 일어나면 QR 이 찍혀 있는가?" **`detail`은 항상 필수**이고 하한이 분류별로 다르다 (공개 150자 / 참여자 80자, 상한 500). **관리자는 자기 프로그램의 신고도 읽을 수 없다** — 신고자를 특정하면 보복 경로가 생긴다. 취소·수정 정책 0개 |
...
**참여 팝업 맨 아래에 '신고하기'가 있다 (ADR 0025 → 0026).** 관리자를 견제하는 유일한 학생측 경로다 — 서로 다른 학생 3명이 신고하면 서버가 그 프로그램을 자동으로 내린다(사람이 처리하지 않는다). 사유는 **참여자 전용 2종**(설명 불일치·시간 미준수 — QR 입장 인증을 마친 학생만)과 **공개 5종**(그 외 전부 — 누구나)으로 갈린다. 참여자 전용 항목은 **숨기지 않고 잠근 채로** 보여준다 — 목록 자체가 규칙을 설명한다. >>> 신고 수·임계치 진행률을 화면에 그리지 말 것(원칙 1), 관리자에게 신고자를 알려주지 말 것, `applied`(신청만 함)를 참여 자격에 더하지 말 것.
...
**게시(올리기)는 세 관문을 지난다 (ADR 0026).** ① 학생 목록에 보일 모습 미리보기 ② 원칙 체크리스트 3문장(학업 아님·무료·부적절하지 않음) ③ 서버 검사(`publish_my_program()` — 설명 50자↑, 진행이 끝나지 않음, 남은 진행일 있음). **그 체크리스트 3문장은 공개 신고 사유 3종과 정확히 같다** — "네가 올리며 확인한 것이 곧 학생이 신고할 수 있는 것"이라서, 한쪽을 바꾸면 다른 쪽도 바꿔야 한다. 트리거 `programs_publish_gate`가 RPC를 거치지 않은 `false → true` 전이를 막으므로 화면 우회로도 게시되지 않는다. **내리기는 그대로 평범한 update다** — 문턱은 올리는 쪽에만 있다(`stale` 알림이 요구하는 행동이 '내리기'라서).

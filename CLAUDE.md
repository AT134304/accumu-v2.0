...
| `program_reports` | id, program_id, student_id, reason, detail | 학생의 프로그램 신고 (ADR 0025). **서로 다른 학생 3명**이면 서버가 자동으로 게시중단(삭제 아님). 학생 1명당 1건(unique). **사유가 두 종류다(ADR 0026)**: 공개(`irrelevant`·`paid`·`inappropriate` — 공고만 봐도 아는 것, 누구나) / 참여자 전용(`not_real`·`mismatch`·`unpunctual`·`other` — `status in (entered, completed)` 인 학생만). **`detail`은 항상 필수**이고 하한이 분류별로 다르다 (공개 150자 / 참여자 80자, 상한 500). **관리자는 자기 프로그램의 신고도 읽을 수 없다** — 신고자를 특정하면 보복 경로가 생긴다. 취소·수정 정책 0개 |
...
**참여 팝업 맨 아래에 '신고하기'가 있다 (ADR 0025 → 0026).** 관리자를 견제하는 유일한 학생측 경로다 — 서로 다른 학생 3명이 신고하면 서버가 그 프로그램을 자동으로 내린다(사람이 처리하지 않는다). 사유는 **공개 3종**(공고만 봐도 아는 것 — 누구나)과 **참여자 전용 4종**(겪어야 아는 것 — QR 입장 인증을 마친 학생만)으로 갈린다. 참여자 전용 항목은 **숨기지 않고 잠근 채로** 보여준다 — 목록 자체가 규칙을 설명한다. >>> 신고 수·임계치 진행률을 화면에 그리지 말 것(원칙 1), 관리자에게 신고자를 알려주지 말 것, `applied`(신청만 함)를 참여 자격에 더하지 말 것.

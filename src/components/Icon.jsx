...
const ICONS = {
  ...
  // 가로 모드 안내 — 눕힌 화면(흐림)에서 세운 화면(진함)으로. 화살표 없이 두 상태만으로 읽힌다.
  'ic-rotate': {
    content: (
      <>
        <rect x="2.4" y="8.6" width="19.2" height="9.4" rx="2.2" opacity=".34" />
        <rect x="8.6" y="2.4" width="6.8" height="19.2" rx="2.2" />
      </>
    ),
  },
  // 프로그램 대표 사진 (ADR 0022) — 업로드 버튼 / 사진 없음 자리표시
  'ic-image': {
    content: (
      <>
        <rect x="3" y="5" width="18" height="14" rx="2.8" opacity=".34" />
        <circle cx="8.4" cy="9.8" r="1.7" />
        <path d="M4.2 17.2l3.9-3.9 2.7 2.7 3.5-3.9 5.5 5.9c-.3.6-1 1-1.8 1H6a2 2 0 0 1-1.8-1.8z" />
      </>
    ),
  },
  ...
  // 내리기 (게시중단)
  // 잠금 — 끝난 프로그램의 "수정" 버튼(ADR 0025). 몸통이 진하고 고리가 연한 duotone.
  'ic-lock': {
    content: (
      <>
        <path
          opacity=".34"
          d="M8.2 10.4V7.9a3.8 3.8 0 0 1 7.6 0v2.5h-2.1V7.9a1.7 1.7 0 0 0-3.4 0v2.5H8.2z"
        />
        <rect x="4.6" y="10.2" width="14.8" height="10.4" rx="2.6" />
        {/* 열쇠구멍 — 다른 아이콘의 흰색 하이라이트와 같은 계열(배경색과 무관하게 형태가 읽힌다) */}
        <circle cx="12" cy="14.6" r="1.5" fill="#fff" />
        <rect x="11.25" y="15.3" width="1.5" height="3" rx=".75" fill="#fff" />
      </>
    ),
  },

...
//
// [★ 신고는 두 종류다 — ADR 0026 / 2026-08-22 개정]
//   참여자(participant): mismatch(설명과 다름) / unpunctual(시간 미준수) 2종.
//                        QR 입장 인증을 마친 학생만 신고할 수 있다.
//   공개(open)         : 그 외 전부. 참여하지 않은 학생도 신고할 수 있다.
//
//   [가르는 기준] "공고만 봐도 아는가"가 아니라 **"참여 인증(QR)이 실제로 남을 수 있는 일인가"** 다.
//     처음에는 not_real(안 열림)을 참여자 전용에 뒀는데, 프로그램이 안 열리면 QR 을 찍어 줄 관리자도
//     없어서 status 가 영원히 applied 다 — **아무도 쓸 수 없는 죽은 사유**였다(케빈 지적).
//     >>> 새 사유를 추가할 때 먼저 물을 것: "이 일이 일어나면 QR 이 찍혀 있는가?" 아니면 공개다.
//
//   그리고 공개 사유 중 **irrelevant / paid / inappropriate 3종은 관리자가 게시 전에 체크하는
//   3문장과 정확히 같다**(PublishConfirm). "네가 올리며 확인한 것이 곧 학생이 신고할 수 있는 것"
//   이라서, 신고가 취향 차이가 아니라 **약속 위반**을 가리킨다.
//   >>> 이 셋을 바꾸면 AdminProgramsPage 의 체크리스트도 함께 바꿀 것.
//   (not_real / other 는 그 대칭 밖이다 — 관리자가 미리 약속할 수 있는 성질의 일이 아니다.)
//
// [★ 이 파일이 절대 하지 않는 것]
 ...
 *  [순서] 공개 5종을 먼저 둔다 — 누구에게나 열린 항목이 위에 있어야 잠긴 항목이 예외로 읽힌다. */
export const REPORT_REASONS = [
  ...
  // [참여자 전용이 아니다 — 2026-08-22] 프로그램이 안 열렸으면 QR 을 찍어 줄 관리자도 없다.
  //   참여자 전용에 두면 자격이 영원히 생기지 않아 아무도 못 쓰는 사유가 된다.
  { key: 'not_real', scope: 'open', label: '실제로 열리지 않았어요', hint: '공고된 날짜에 진행되지 않음' },
  // [기타도 공개다] 정의상 무엇인지 미리 알 수 없어 자격을 좁힐 근거가 없다.
  //   참여자 전용 사유를 기타로 우회할 수는 있지만, 공개 하한(150자)이 참여자 하한(80자)보다
  //   비싸서 우회가 손해다 — 구멍이 아니라 값비싼 우회로다.
  { key: 'other', scope: 'open', label: '기타', hint: '위 항목에 해당하지 않는 문제' },
  {
    key: 'mismatch',
    scope: 'participant',

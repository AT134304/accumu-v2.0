// Accumu v2 — 네이버 로그인 콜백 (/auth/naver)
//
// 네이버가 code + state 를 붙여 이 주소로 돌려보낸다. 여기서 Edge Function 을 호출해 세션으로 바꾼다.
//
...
import { completeNaverLogin, naverReasonText } from '../lib/authService';
...
export default function NaverCallbackPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      // 사용자가 네이버 화면에서 취소하면 code 대신 error 가 온다.
      if (params.get('error')) {
        setError(naverReasonText(params.get('error') === 'access_denied' ? 'denied' : 'unknown'));
        return;
      }

      const result = await completeNaverLogin({
        code: params.get('code'),
        state: params.get('state'),
      });

      if (!result.ok) {
        setError(naverReasonText(result.reason));
        return;
      }
      ...
      // 네이버 계정은 언제나 학생이다(Edge Function + 트리거가 그렇게 고정한다).
